...
function getRandomColor(isPositive) {
  // Set colors for positive and negative objects
  const positiveColors = [0x00ff00, 0x0000ff, 0xffff00, 0xffa500]; // green, blue, yellow, orange
  const negativeColors = [0xff0000, 0x800080]; // red, purple

  if (isPositive) {
    const randomIndex = Math.floor(Math.random() * positiveColors.length);
    return new THREE.Color(positiveColors[randomIndex]);
  } else {
    const randomIndex = Math.floor(Math.random() * negativeColors.length);
    return new THREE.Color(negativeColors[randomIndex]);
  }
}
...
class FloatingObject {
  constructor(_scene, isPositive) {
    ...
    // Adjust the spawn range to make the sea feel more crowded
    ...
    const color = getRandomColor(isPositive);
...
let floatingObjects = [];
const OBJECT_COUNT = 500; // Increased number of objects
init();
animate();
...
function createStartScreen() {
  ...
  instructions.textContent = 'Use Arrow keys to move the boat. Collect green,yellow and orange algae, avoid red and purple algae!';
...
async function init() {
  ...
  // Add lighting for natural effects
  const ambientLight = new THREE.AmbientLight(0x404040, 2); // Soft ambient light
  scene.add(ambientLight);

  const directionalLight = new THREE.DirectionalLight(0xffffff, 1);
  directionalLight.position.set(100, 100, 100).normalize(); // Light from above
